...
import '/src/Pages/Icecream/Icecream.css'
...
function Icecream({setCartId ,setProductInfoId}) {

    const [icecream, setIceCream] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        const filterFastfood = groceryData.filter((items) => items.category === "IceCream");
        setIceCream(filterFastfood);
    }, [])
    ...
    return (
        ...
        <div>
            ...
            <div className="container-fluid bg-transparent">
                <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/icecream" className="text-success">ICECREAM</NavLink></div>
                        ...
                        icecream.map((product) => {
                            return (
                                <figure key={product.id} className="col-5 col-md-2 border rounded">
                                    <img src={product.image} alt={product.name} className="icecream-list col-12" onClick={() => onImageClicked(product.id)}/>
