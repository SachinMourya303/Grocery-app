...
import '/src/Pages/Oatmeal/Oatmeal.css'
...
function Oatmeal({setCartId , setProductInfoId}) {
    const [oatmeal, setOatmeal] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        const filterFastfood = groceryData.filter((items) => items.category === "Instant");
        setOatmeal(filterFastfood);
    }, [])
    ...
    return (
        ...
        <div>
            ...
            <div className="container-fluid bg-transparent">
                <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/oatmeal" className="text-success">OATS</NavLink></div>
                        ...
                        oatmeal.map((product) => {
                            return (
                                <figure key={product.id} className="col-5 col-md-2 border rounded">
                                    <img src={product.image} alt={product.name} className="oats-list col-12" onClick={() => onImageClicked(product.id)}/>
