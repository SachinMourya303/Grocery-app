...
import '/src/Pages/Fruits/Fruits.css'
...
function Fruits({setCartId , setProductInfoId}) {

    const [fruits, setFruits] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        const filterFastfood = groceryData.filter((items) => items.category === "Fruits");
        setFruits(filterFastfood);
    }, [])
    ...
    return (
        ...
        <div>
            ...
            <div className="container-fluid bg-transparent">
                <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/fruits" className="text-success">FRUITS</NavLink></div>
                        ...
                        fruits.map((product) => {
                            return (
                                <figure key={product.id} className="col-5 col-md-2 border rounded">
                                    <img src={product.image} alt={product.name} className="fruits-list col-12" onClick={() => onImageClicked(product.id)}/>
                                    <div className="captions">
                                        <figcaption className="text-secondary ms">{product.category}</figcaption>
