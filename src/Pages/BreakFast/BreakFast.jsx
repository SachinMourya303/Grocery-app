...
import "/src/Pages/BreakFast/BreakFast.css"
import '/src/Pages/AllProducts/AllProducts.css'
...
const BreakFast = ({ setCartId, setProductInfoId }) => {

    const [breakFast, setBreakFast] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        const filterBreakfast = groceryData.filter((items) => items.type === "breakfast");
        ...
        <>
            <div>
                ...
                <div className="container-fluid bg-transparent">
                    <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/breakfast" className="text-success">BREAKFAST</NavLink></div>
                            ...
                            breakFast.map((product) => {
                                return (
                                    <figure key={product.id} className="col-5 col-md-2 border rounded">
                                        <img src={product.image} alt={product.name} className="col-12 breakfast-list" onClick={() => onImageClicked(product.id)} />
