...
import '/src/Pages/Grains/Grains.css'
...
function Grains({setCartId , setProductInfoId}) {

    const [grains, setGrains] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        const filterFastfood = groceryData.filter((items) => items.category === "Grains");
        setGrains(filterFastfood);
    }, [])
    ...
    return (
        ...
        <div>
            ...
            <div className="container-fluid bg-transparent">
                <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/grains" className="text-success">GRAINS</NavLink></div>
                        ...
                        grains.map((product) => {
                            return (
                                <figure key={product.id} className="col-5 col-md-2 border rounded">
                                    <img src={product.image} alt={product.name} className="grains-list col-12" onClick={() => onImageClicked(product.id)}/>
