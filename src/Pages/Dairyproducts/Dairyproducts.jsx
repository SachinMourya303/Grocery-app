...
import '/src/Pages/Dairyproducts/Dairyproducts.css'
...
function Dairyproducts({setCartId ,setProductInfoId}) {

    const [bakery, setBakery] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        const filterFastfood = groceryData.filter((items) => items.category === "Bakery");
        setBakery(filterFastfood);
    }, [])
    ...
    return (
        ...
        <div>
            ...
            <div className="container-fluid bg-transparent">
                <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/dairyproducts" className="text-success">DAIRY</NavLink></div>
                <div className="d-flex col-12 flex-wrap gap-3 gap-md-5 justify-content-md-strat justify-content-center mt-3">
                    {
                        bakery.map((product) => {
                            return (
                                <figure key={product.id} className="col-5 col-md-2 border rounded">
                                    <img src={product.image} alt={product.name} className="dairy-list col-12" onClick={() => onImageClicked(product.id)}/>
