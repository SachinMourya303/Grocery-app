...
import '/src/Pages/Drinks/Drinks.css'
...
function Drinks({setCartId , setProductInfoId}) {

    const [drinks, setDrinks] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        const filterFastfood = groceryData.filter((items) => items.category === "Drinks");
        setDrinks(filterFastfood);
    }, [])
    ...
    return (
        ...
        <div>
            ...
            <div className="container-fluid bg-transparent">
                <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/drinks" className="text-success">DRINKS</NavLink></div>
                <div className="d-flex col-12 flex-wrap gap-3 gap-md-5 justify-content-start justify-content-center mt-3">
                    {
                        drinks.map((product) => {
                            return (
                                <figure key={product.id} className="col-5 col-md-2 border rounded">
                                    <img src={product.image} alt={product.name} className="drinks-list col-12" onClick={() => onImageClicked(product.id)}/>
