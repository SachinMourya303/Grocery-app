...
import '/src/Pages/Vegitables/Vegitables.css'
...
function Vegitables({setCartId , setProductInfoId}) {


    const [vegitables, setVegitables] = useState([]);
    const navigate = useNavigate();
    useEffect(() => {
        const filterFastfood = groceryData.filter((items) => items.category === "Vegetables");
        setVegitables(filterFastfood);
    }, [])
    ...
    return (
        ...
        <div>
            ...
            <div className="container-fluid bg-transparent">
                <div><NavLink to="/" className="bi bi-house text-success"> / </NavLink><NavLink to="/vegitables" className="text-success">VEGITABLES</NavLink></div>
                        ...
                        vegitables.map((product) => {
                            return (
                                <figure key={product.id} className="col-5 col-md-2 border rounded">
                                    <img src={product.image} alt={product.name} className="vegitables-list col-12" onClick={() => onImageClicked(product.id)}/>
